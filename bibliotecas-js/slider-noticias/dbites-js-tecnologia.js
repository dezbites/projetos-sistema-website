...
const _ = {
	...
	},sliderNot:function(imgsQ){
		var movQt   = 0;
		var imgs    = imgsQ;

		var htmlSlidenoti = "<div>";
		

			for(var i = 0; i < imgs.length; i++){
					htmlSlidenoti += "<div><img src="+imgs[i][0]+" /><h2>"+imgs[i][1]+"</h2><p>"+imgs[i][2]+"</p><p>"+imgs[i][3]+"</p></div>";
				
			}
			
			htmlSlidenoti += "</div><span class='dbites-slide-controle'><div>&nbsp;</div><div>&nbsp;</div></span>";

		

		var scro       = document.querySelector(".dbites-slide-noticias");
		scro.innerHTML = htmlSlidenoti;
		var divcon     = scro.querySelector("div");
		var control    = document.querySelectorAll(".dbites-slide-controle>div");

			control[0].onclick = function(){
				divcon.style.marginLeft = "-0%";
				control[0].style.backgroundColor = "#4580c5";
				control[1].style.backgroundColor = "#cccccc";
			}
			control[1].onclick = function(){
				divcon.style.marginLeft = "-50%";
				control[1].style.backgroundColor = "#4580c5";
				control[0].style.backgroundColor = "#cccccc";
			}
			/*setInterval(function(){
				divcon.style.marginLeft = "-"+movQt+"%";
				if( movQt == 0 ){
					control[0].style.backgroundColor = "#4580c5";
					control[1].style.backgroundColor = "#cccccc";
				}else{
